...
#| eval: false
library(readODS)
read_ods("data/visas-summary-dec-2023-tables.ods", 
         sheet = "Vis_06",
         skip = 1)
...
country_level_2022 %>% 
  ...
  mutate(
    un_subregion_name = countryname(
      sourcevar = country_name,
      destination = "un.regionsub.name"
      )
  ) %>% 
  mutate(
    un_subregion_name = un_subregion_name %>% 
      {
        case_when(
          . %in% c(
            "Sub-Saharan Africa",
  ...
  ) %>% # write_csv("data/export-for-fede.csv") # count(un_subregion_name)
  filter( 
    ! un_subregion_name %in% c(
      "Western Europe", "Southern Europe", "Northern Europe"
    )
  ) %>% 
  ggplot() +
  aes(x = ratio_rejected,
      y = reorder(un_subregion_name, ratio_rejected, FUN=median)) +
  geom_boxplot(
    outlier.alpha = 0
  ) +
  geom_jitter(
    aes(colour = log(gdp_per_capita)),
    width = 0, 
    height = .1,
    size = 3
    ) +
  stat_summary(
    fun = mean, geom = "text", label = "I", colour = "red", size = 10) +
